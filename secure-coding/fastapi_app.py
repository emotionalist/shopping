...
def add_purchase_record(conn, user_id, product_id, quantity, purchase_date):
    cursor = conn.cursor()
    cursor.execute('INSERT INTO purchases (user_id, product_id, quantity, purchase_date) VALUES (?, ?, ?, ?)',
...
async def get_purchases(user_id: int):
    ...
    conn.close()
    return [{"product_id": purchase[2], "quantity": purchase[3], "purchase_date": purchase[4]} for purchase in purchases]
