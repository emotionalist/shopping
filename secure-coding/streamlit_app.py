        ...
        if st.session_state.user["role"] == 'admin':
            ...
            if choice == 'Home':
                ...
                for product in products:
                    ...
                    with col2:
                        if st.button(f"Delete {product['id']}"):
                            delete_response = requests.delete(f'http://localhost:8000/delete_product/{product["id"]}')
            ...
            if choice == 'Home':
                ...
                if st.button('View Purchase History'):   
                    ...
                    response = requests.get(f'http://localhost:8000/purchases?user_id={user_id}')        
                    if response.status_code == 200:
                        purchase_history = response.json()
                        for purchase in purchase_history:
                            st.write(f"Product ID: {purchase['product_id']}, Quantity: {purchase['quantity']}, Date: {purchase['purchase_date']}")
                    else:
                        st.error("Failed to retrieve purchase history.")
